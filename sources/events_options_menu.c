...
options_t *options)
{
    ...
    entities[0]->position.x + entities[0]->texture_rect.width) {
        if (options->sound_off == 0)
            ...
            options->sound_off = 0;
    } if (mouse_info->rpos.x >= entities[1]->position.x && mouse_info->rpos.x <= 
...
{
    if (!(mouse_info->rpos.y >= entities[4]->position.y && mouse_info->rpos.y <= 
...
options_t *options, sfRenderWindow *window)
{
    if (!(mouse_info->rpos.y >= entities[8]->position.y && mouse_info->rpos.y <= 
    ...
    } if (mouse_info->rpos.x >= entities[11]->position.x && mouse_info->rpos.x 
